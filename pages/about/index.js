      ...
      <div className='container mx-auto h-full flex flex-col items-center xl:flex-row gap-x-6'>
        <div className='flex-1 flex flex-col justify-center'>
          ...
          >
            ...
            remote work for agencies, counsulted for startups, and collaborated on digital
            products for bussiness and consumer use.
          </motion.p>
          <motion.div
        ...
        >
          <div className='flex gap-x-4 xl:gap-x-8 mx-auto xl:mx-0 mb-4'>
            {aboutData.map((aboutItem, aboutItemIndex) => (
              <div
                key={aboutItemIndex}
                className={`${index === aboutItemIndex ?
          ...
          <div className='py-2 xl:py-6 flex flex-col gap-y-2 xl:gap-y-4 items-center xl:items-start'>
            {aboutData[index].info.map((aboutDataInfo, aboutDataInfoIndex) => (
              <div
                key={aboutDataInfoIndex}
                className='flex-1 flex flex-col md:flex-row max-w-max gap-x-2 items-center text-white/60'
              >
                <div className='font-light mb-2 md:mb-0'>{aboutDataInfo.title}</div>
                <div className='hidden md:flex'>-</div>
                <div>{aboutDataInfo.stage}</div>
                <div className='flex gap-x-4'>
                  {aboutDataInfo.icons?.map((icon, iconIndex) => (
